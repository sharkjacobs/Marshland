...
class ActionProcessor {
    ...
    private func perform(edit: Edit) {
        switch edit {
        ...
        case .deletePreservingIndentation(range: let range):
            ...
            if change != 0 {
                perform(edit: .indentLocation(location: range.location, depth: change))
            }
            perform(edit: .delete(range: range))
        case .indentRange(range: let range, depth: let depth):
            ...
            content.enumerateSubstrings(in: range, options: .byLines) {
                ...
                if actualDepth != 0 {
                    self.perform(edit: .indentLocation(location: range.location, depth: actualDepth))
                }
            }
...
extension UndoGroupingStateMachine {
    ...
    /// - Parameter action: Invoked user action
    /// - Returns: Decision whether the current undo group should end and a new one begin
    func processAction(_ action: ActionProcessor.Action) -> Bool {
        ...
        let decision = processEvent(event)

        return decision != .continueCurrentGroup
    }
}
